import { CreateTableNode, CreateViewNode, DefaultQueryCompiler, IdentifierNode, ON_COMMIT_ACTIONS, RawNode, SchemableIdentifierNode, freeze, isBigInt, isBoolean, isBuffer, isDate, isFunction, isNull, isNumber, isObject, isString, isUndefined, noop } from "./default-query-compiler-muNzRcml.mjs";
import { CaseBuilder, CaseNode, ColumnNode, CompiledQuery, Deferred, DynamicReferenceBuilder, DynamicTableBuilder, OperationNodeTransformer, QueryCreator, QueryExecutorBase, QueryNode, SelectAllNode, SelectQueryNode, ValueListNode, ValueNode, WithSchemaPlugin, createFunctionModule, createQueryId, isOperationNodeSource, logOnce, parseColumnName, parseDataTypeExpression, parseExpression, parseOrderedColumnName, parseStringReference, parseTable, parseValueBinaryOperationOrExpression, printBackgroundFail, provideControlledConnection, randomString, sql, waitOrAbort } from "./compiled-query-WAoO9V3N.mjs";
import { DialectAdapterBase, SqliteAdapter, SqliteQueryCompiler } from "./sqlite-adapter-COq13qXk.mjs";
import { createAdapterFactory, logger } from "./factory-DKOC6c1p.mjs";
import { capitalizeFirstLetter } from "./string-F0KIiaBX.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/dist-5KV-bOtK.js
/**
* @internal
*/
...
var createKyselyAdapter = async (config) => {
	...
	if ("fileControl" in db) {
		const { BunSqliteDialect } = await import("./bun-sqlite-dialect-BW9W1_Ps-DEGuLtOB.mjs");
	...
	if ("createSession" in db) {
		...
		if (DatabaseSync && db instanceof DatabaseSync) {
			const { NodeSqliteDialect } = await import("./node-sqlite-dialect-C9mZn4qI.mjs");
			dialect = new NodeSqliteDialect({ database: db });
		}
	}
	if ("batch" in db && "exec" in db && "prepare" in db) {
		const { D1SqliteDialect } = await import("./d1-sqlite-dialect-BLC8LXE6-DNYuM_yd.mjs");
